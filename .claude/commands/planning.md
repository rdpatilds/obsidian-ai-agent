...
- `src/shared/middleware.py` - Request logging middleware
- `pyproject.toml` - Dependencies and tool configuration
- `ruff.toml` - Linting rules (120 line length, strict checks)

**Project Structure:**

```
src/
├── agent/          # Core agent orchestration (Pydantic AI)
├── openai/         # OpenAI compatibility layer (FastAPI endpoints)
├── tools/          # Vertical slices for tools
│   ├── web_search/
│   ├── image_analysis/
│   ├── video_generation/
│   └── obsidian_mcp/
...
<list commands you'll use to validate with 100% confidence the feature is implemented correctly with zero regressions. Include:
...
- Start server: `uv run uvicorn src.main:app --host 0.0.0.0 --port 8030`
